...
class type_whole_overlap:
    def __init__(self):
        ...
        self.term_frequency_csv_list = os.listdir(
            os.path.join(self.dir_path, 'res/term_frequency'))

    def type_overlap(self):
        term_list_csv = []
        for csv in self.term_frequency_csv_list:
            term_list_i = set(list(pd.read_csv(open(os.path.join(
                self.dir_path+'/res/term_frequency', csv), encoding='utf-8-sig'), header=0)['term']))
            term_list_csv.append(term_list_i)
        overlap_list = []

        # 将任意的两个csv_list 进行比较，看是否有重合的term
        for i in range(len(term_list_csv)):
            for j in range(i+1, len(term_list_csv)):
                ...
                for iii in overlap_term:
                    # 这里面的term_frequency_csv_list[i, j] 表示的是属于哪一的文件的
                    overlap_list.append(
                        [iii, self.term_frequency_csv_list[i], self.term_frequency_csv_list[j]])
        nodup = pd.DataFrame(overlap_list)
        nodup = nodup.reset_index()
        if(len(nodup) > 0):
            nodup.columns = ['index', 'term', 'label_type_1', 'label_type_2']
        nodup.to_csv(self.dir_path+'/res/whole_overlap.csv',
                     encoding='utf-8-sig', index=False)


# 分析类别内部存在的overlap
# 例如一个term 叫做 “浅表性胃炎”，另一个叫做 “重度慢性浅表性胃炎” 此两个之间具有关系
class overlap_intype:
    ...
    def overlap(self):
        for csv in self.term_frequency_csv_list:
            res = []
            term_list = set(list(pd.read_csv(open(os.path.join(
                self.dir_path+'/res/term_frequency', csv), encoding='utf-8-sig'), header=0)['term']))
            ...
            res_df.to_csv(os.path.join(self.dir_path+'/res/overlap_intype', csv).replace(
                'term_frequency.csv', 'overlap.csv'), index=False, encoding='utf-8-sig')
...
class overlap_betweentype:
    ...
    def type_overlap(self, file_path1, file_path2):
        term_list1 = set(
            list(pd.read_csv(open(file_path1, encoding='utf-8-sig'), header=0)['term']))
        term_list2 = set(
            list(pd.read_csv(open(file_path2, encoding='utf-8-sig'), header=0)['term']))
        res1 = []
        res2 = []
        ...
        writer = pd.ExcelWriter(os.path.join(
            self.dir_path, 'res/overlap_betweentype/'+csv1+'+'+csv2+'.xlsx'))
        res_df1.to_excel(writer, sheet_name=csv1+'&'+csv2,
                         index=False, encoding='utf-8-sig')
        res_df2.to_excel(writer, sheet_name=csv2+'&'+csv1,
                         index=False, encoding='utf-8-sig')
        writer.save()
    ...
    def main_overlap_between(self):
        for csv_i in range(len(self.term_frequency_csv_list)):
            file_path_i = os.path.join(
                self.dir_path, 'res/term_frequency')+'//'+self.term_frequency_csv_list[csv_i]
            for csv_j in range(csv_i+1, len(self.term_frequency_csv_list)):
                file_path_j = os.path.join(
                    self.dir_path, 'res/term_frequency')+'//'+self.term_frequency_csv_list[csv_j]
